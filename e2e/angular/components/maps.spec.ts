...
test.describe('Maps', () => {
  test('Default story renders a Leaflet map with tiles and zoom controls without console errors', async ({
    ...
    await page.goto('?path=/story/components-data-display-maps--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const map = frame.getByTestId('maps')
  ...
  test('zoom-in control is clickable and does not error', async ({
    page
  }) => {
    const errors: string[] = []
    page.on('pageerror', (error) => errors.push(error.message))
