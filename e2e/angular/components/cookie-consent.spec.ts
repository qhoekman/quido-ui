...
const STORY_URL = '?path=/story/components-feedback-cookie-consent--default'
...
test.describe('Cookie Consent', () => {
  ...
  test('shows the banner when no consent is stored yet, no console errors', async ({ page }) => {
    ...
    await expect(frame.getByTestId('cookie-consent')).toBeVisible()
  ...
  test('regression: clicking Accept hides the banner and persists consent across reload', async ({
    ...
    await frame.getByTestId('cookie-consent__accept').click()
    await expect(frame.getByTestId('cookie-consent')).toHaveCount(0)

    const stored = await frame
      .locator('body')
      .evaluate(() => window.localStorage.getItem('cookieConsent'))
  ...
  test('clicking Reject hides the banner and persists rejection', async ({ page }) => {
    ...
    await frame.getByTestId('cookie-consent__reject').click()
    await expect(frame.getByTestId('cookie-consent')).toHaveCount(0)
  ...
  test('clicking the close (X) button also dismisses via reject', async ({ page }) => {
    ...
    await frame.getByTestId('cookie-consent__close').click()
    await expect(frame.getByTestId('cookie-consent')).toHaveCount(0)
