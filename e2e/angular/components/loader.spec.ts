...
test.describe('Loader', () => {
  test('Default story renders active with the spinner icon visible, no console errors', async ({
    ...
    const loader = frame.getByTestId('loader')
  ...
  test('regression: active=false hides the loader (opacity 0) and removes the spinner icon', async ({
    page
  }) => {
    await page.goto(
  ...
  test('toggling active live correctly transitions opacity', async ({ page }) => {
    await page.goto('?path=/story/components-feedback-loader--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const loader = frame.getByTestId('loader')
    ...
    const activeControl = page.locator('#control-active')
