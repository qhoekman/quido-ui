...
test.describe('Loading Bar', () => {
  test('Default story renders the indicator with the indeterminate animation, no console errors', async ({
    ...
    await page.goto('?path=/story/components-feedback-loading-bar--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const bar = frame.getByTestId('loading-bar')
  ...
  test('loading=false renders the bar without the indicator', async ({ page }) => {
    await page.goto(
      '?path=/story/components-feedback-loading-bar--default&args=loading:false'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')

    await expect(frame.getByTestId('loading-bar')).toBeVisible()
