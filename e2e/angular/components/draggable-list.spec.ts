...
test.describe('DraggableList', () => {
  test('Default story renders all items as a list, no console errors', async ({
    page
  }) => {
    const errors: string[] = []
    ...
    await page.goto(
      '?path=/story/components-data-entry-draggable-list--default'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const list = frame.getByTestId('draggable-list')
    const items = frame.getByTestId('draggable-list__item')
  ...
  test('regression: dragging the first item over the last reorders the list and emits reorder', async ({
    ...
    const dataTransfer = await items
      .nth(0)
    ...
    await expect
      .poll(() => consoleLogs.some((log) => log.includes('New order')))
      .toBe(true)
  })
})
