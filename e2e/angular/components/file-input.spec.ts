...
test.describe('FileInput', () => {
  test('Default story renders the placeholder, no console errors', async ({
    page
  }) => {
    const errors: string[] = []
    ...
    const trigger = frame.getByTestId('file-input')
  ...
  test('selecting a file shows the selected count and emits filesSelected', async ({
    page
  }) => {
    await page.goto('?path=/story/components-data-entry-file-input--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const trigger = frame.getByTestId('file-input')
    const input = frame.getByTestId('file-input__input')
  ...
  test('multiple prop sets the native multiple attribute', async ({
    page
  }) => {
    await page.goto(
