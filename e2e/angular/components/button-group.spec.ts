...
test.describe('ButtonGroup', () => {
  test('Default story renders three joined buttons without console errors', async ({
    page
  }) => {
    const errors: string[] = []
    page.on('pageerror', (error) => errors.push(error.message))

    await page.goto('?path=/story/components-actions-button-group--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const group = frame.getByTestId('button-group')
  ...
  test('only the first and last buttons have outer rounded corners', async ({
    page
  }) => {
    await page.goto('?path=/story/components-actions-button-group--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const buttons = frame.getByTestId('button-group').getByRole('button')
