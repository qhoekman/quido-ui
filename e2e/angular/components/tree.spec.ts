...
test.describe('Tree', () => {
  test('Default story renders folders with correctly indented children, no console errors', async ({
    ...
    await page.goto('?path=/story/components-data-display-tree--default')
    ...
    const parentBox = await frame
      .getByTestId('tree__item-trigger')
      .first()
      .boundingBox()
    const groupBox = await frame.getByTestId('tree__group').first().boundingBox()
  ...
  test('clicking a collapsed folder trigger expands it and shows its children', async ({
    ...
    await expect(
      projectsTrigger.locator('xpath=ancestor::*[@role="treeitem"][1]')
