...
test.describe('Input', () => {
  ...
  test('regression: disabled/required/readOnly/maxLength/minLength props are applied to the DOM', async ({
    ...
    await page.goto(
      '?path=/story/components-data-entry-input--default&args=disabled:true;required:true;readOnly:true;maxLength:5;minLength:2'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const input = frame.locator('.q-input')
  ...
  test('type prop switches the native input type', async ({ page }) => {
    await page.goto(
      '?path=/story/components-data-entry-input--default&args=type:email'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
