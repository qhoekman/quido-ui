...
test.describe('ColorPalette', () => {
  test('Default story renders 246 color swatches without console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-color-palette--default')
  ...
  test('WithSelectedColor story pre-selects the given color', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-color-palette--with-selected-color'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
