...
test.describe('Checkbox', () => {
  test('Default story renders unchecked without console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-checkbox--default')
  ...
  test('regression: an explicit checked=true prop is honored (was previously silently ignored)', async ({
    ...
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-checkbox--default&args=checked:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const checkbox = frame.locator('.q-checkbox')
  ...
  test('disabled checkbox cannot be toggled', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-checkbox--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const checkbox = frame.locator('.q-checkbox')
