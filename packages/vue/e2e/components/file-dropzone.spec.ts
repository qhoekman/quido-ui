...
test.describe('File Dropzone', () => {
  test('Default story renders the dropzone with slot content, no console errors', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-file-dropzone--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const dropzone = frame.getByTestId('qui-file-dropzone')
  ...
  test('clicking the dropzone opens the native file chooser', async ({ page }) => {
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-file-dropzone--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const dropzone = frame.getByTestId('qui-file-dropzone')
  ...
  test('selecting a file via the hidden input adds it to the file list and emits filesSelected', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-file-dropzone--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const input = frame.getByTestId('qui-file-dropzone-input')
    ...
    const items = frame.getByTestId('qui-file-dropzone-file-item')
  ...
  test('removing a file clears the file list', async ({ page }) => {
    ...
    const item = frame.getByTestId('qui-file-dropzone-file-item')
    await expect(item).toHaveCount(1)
    await item.getByRole('button').click()

    await expect(frame.getByTestId('qui-file-dropzone-file-list')).toHaveCount(0)
  })
})
