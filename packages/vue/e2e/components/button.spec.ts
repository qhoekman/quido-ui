...
test.describe('Button', () => {
  test('Default story loads without console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/components-actions-button--default')
  ...
  test('defaults to a real, focusable <button> element even without an explicit "as" prop', async ({
    ...
    await page.goto(
      'http://localhost:6006/iframe.html?id=components-accessibility-visually-hidden--default&viewMode=story'
    )
  ...
  test('disabled state applies aria-disabled and native disabled', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-actions-button--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
