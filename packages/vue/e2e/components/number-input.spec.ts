...
test.describe('Number Input', () => {
  test('Default story renders with the initial value, no console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/features-ecommerce-components-number-input--default')
  ...
  test('disabled prop prevents increment/decrement', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/features-ecommerce-components-number-input--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const input = frame.locator('input[type="number"]')
