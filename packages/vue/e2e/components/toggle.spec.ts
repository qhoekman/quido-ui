...
test.describe('Toggle', () => {
  test('Default story loads without console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/components-actions-toggle--default')
  ...
  test('disabled toggle cannot be pressed', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-actions-toggle--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const toggle = frame.getByRole('button', { name: 'Toggle' })
