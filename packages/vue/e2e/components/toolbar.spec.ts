...
test.describe('Toolbar', () => {
  test('WithText story renders a fixed bottom toolbar with 5 links, no console errors', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/features-mobile-components-toolbar--with-text')
  ...
  test('WithIcons story renders 5 icon links', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/features-mobile-components-toolbar--with-icons'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
