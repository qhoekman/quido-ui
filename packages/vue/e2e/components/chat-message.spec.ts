...
test.describe('ChatMessage', () => {
  test('Default story renders sender and receiver bubbles without console errors', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-display-chat-message--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const messages = frame.getByTestId('qui-chat-message')

    await expect(messages).toHaveCount(2)
    await expect(frame.getByText('User Name')).toBeVisible()
    await expect(frame.getByText('John Doe')).toBeVisible()
    await expect(frame.getByTestId('qui-chat-message-bubble')).toHaveCount(2)
  ...
  test('sender variant is row-reverse, receiver variant is row', async ({ page }) => {
    await page.goto('http://localhost:6006/?path=/story/components-data-display-chat-message--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const messages = frame.getByTestId('qui-chat-message')
