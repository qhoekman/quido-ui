...
test.describe('Password Input', () => {
  test('Default story renders a password field, no console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-password-input--default')
  ...
  test('disabled prop disables both the input and the toggle button', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-password-input--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
