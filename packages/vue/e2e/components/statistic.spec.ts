...
test.describe('Statistic', () => {
  test('Default story renders label, value, trend, and helper text without console errors', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-display-statistic--default')
  ...
  test('down variant applies destructive-colored trend styling', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-display-statistic--default&args=variant:down'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
