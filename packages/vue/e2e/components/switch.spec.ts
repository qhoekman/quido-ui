...
test.describe('Switch', () => {
  test('Default story renders unchecked, no console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-switch--default')
  ...
  test('disabled prop prevents toggling', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-switch--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const sw = frame.locator('.q-switch')
