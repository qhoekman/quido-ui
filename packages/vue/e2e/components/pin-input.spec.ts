...
test.describe('Pin Input', () => {
  test('Default story renders 4 empty digit inputs, no console errors', async ({ page }) => {
    const errors: string[] = []
    page.on('pageerror', (error) => errors.push(error.message))

    await page.goto('http://localhost:6006/?path=/story/components-data-entry-pin-input--default')
    const frame = page.frameLocator('#storybook-preview-iframe')
    const digits = frame.getByTestId('qui-pin-input-input')
