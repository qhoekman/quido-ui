...
test.describe('Slider', () => {
  test('Default story renders the thumb at its initial value, no console errors', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-slider--default')
  ...
  test('disabled prop makes the thumb non-interactive', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-slider--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const thumb = frame.locator('.q-slider-thumb')
