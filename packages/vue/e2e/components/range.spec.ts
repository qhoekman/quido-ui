...
test.describe('Range', () => {
  test('Default story renders both thumbs at their initial values, no console errors', async ({
    ...
    await page.goto('http://localhost:6006/?path=/story/components-data-entry-range--default')
  ...
  test('disabled prop makes both thumbs non-interactive', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/components-data-entry-range--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
    const thumbs = frame.locator('.q-range-thumb')
