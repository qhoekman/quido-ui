...
test.describe('ListButton', () => {
  test('Default story renders five full-width buttons without console errors', async ({ page }) => {
    ...
    await page.goto('http://localhost:6006/?path=/story/features-mobile-components-list-button--default')
  ...
  test('disabled state applies aria-disabled and native disabled', async ({ page }) => {
    await page.goto(
      'http://localhost:6006/?path=/story/features-mobile-components-list-button--default&args=disabled:true'
    )
    const frame = page.frameLocator('#storybook-preview-iframe')
