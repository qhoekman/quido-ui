...
For styling in this design system, we focus on using native CSS approaches whenever possible for each framework, aligning with modern best practices and providing improved flexibility. Each framework keeps styles **embedded in the same file as the component** (Vue SFC, Angular component decorator, React `.tsx`):

- **React**: Use [Styled Components](https://styled-components.com/) with co-located `css` / `styled.*` blocks in the same `.tsx` file. React has no native `<style scoped>` or Angular-style component `styles` array in JSX; styled-components is this design system’s embedded scoped-CSS equivalent. Pair it with [CVA](https://cva.style) for variants (`q-*`, `variant--*`, `size--*`) and CSS variables from `packages/react/src/index.css`. Do **not** use CSS Modules (sibling `.module.css` files) or Tailwind utilities for React components in this package — they break single-file embedding or diverge from Vue/Angular. See `packages/react/src/components/button/button.tsx` as the gold standard and `packages/react/REACT.TODO.md` for the conversion tracker.
...
Accessibility can be validated in Storybook using browser devtools and manual keyboard/screen-reader testing. The `@storybook/addon-a11y` addon is not currently installed but can be added per package if needed.
